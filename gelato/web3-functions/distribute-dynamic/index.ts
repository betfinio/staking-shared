...
const abi = parseAbi([
  ...
  "function realStaked() external view returns (uint256)",
...
Web3Function.onRun(async (context: Web3FunctionContext) => {
  ...
  try {
    ...
    const realStakedCalls = poolAddresses.map(pool => ({
      abi: abi,
      address: pool,
      functionName: 'realStaked',
      args: []
    }))
    const balanceCalls = poolAddresses.map(pool => ({
      abi: abi,
    ...
    const realStakedData = await client.multicall({
      contracts: realStakedCalls
    })
    ...
    for (const i in poolAddresses) {
      ...
      const realStaked = realStakedData[i].result as bigint;
      const balance = balanceData[i].result as bigint;

      console.log(poolAddr, realStaked, balance);

      // pool is undistributed if it wasn't distributed in the current cycle or balance is greater than real stake
      if (balance > realStaked) {
